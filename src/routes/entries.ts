...
router.get('/', async (req, res) => {
  ...
  const page = await entries.listTimeline(thoughtId, query);
...
router.post('/', async (req, res) => {
  ...
  const entry = await entries.addEntry(thoughtId, body);
...
router.post('/files', uploadSingleFile, async (req, res) => {
  ...
  const entry = await entries.addEntry(thoughtId, {
    kind: 'file',
    file,
    body: form.body,
    tagIds: form.tagIds,
...
router.get('/:entryId', async (req, res) => {
  ...
  const { entry, downloadUrl } = await entries.getEntryDetail(thoughtId, entryId);
...
router.patch('/:entryId', async (req, res) => {
  ...
  res.json(await entries.updateEntry(thoughtId, entryId, patch));
});

router.delete('/:entryId', async (req, res) => {
  const { thoughtId, entryId } = entryParams.parse(req.params);
  await entries.softDeleteEntry(thoughtId, entryId);
  res.status(204).end();
});

router.post('/:entryId/restore', async (req, res) => {
  const { thoughtId, entryId } = entryParams.parse(req.params);
  res.json(await entries.restoreEntry(thoughtId, entryId));
});

router.put('/:entryId/star', async (req, res) => {
  const { thoughtId, entryId } = entryParams.parse(req.params);
  const { starred } = starBody.parse(req.body);
  res.json(await entries.setEntryStarred(thoughtId, entryId, starred));
});

router.post('/:entryId/tags', async (req, res) => {
  const { thoughtId, entryId } = entryParams.parse(req.params);
  const { tagId } = attachTagBody.parse(req.body);
  res.json(await entries.attachTag(thoughtId, entryId, tagId));
});

router.delete('/:entryId/tags/:tagId', async (req, res) => {
  const { thoughtId, entryId, tagId } = entryTagParams.parse(req.params);
  res.json(await entries.detachTag(thoughtId, entryId, tagId));
});
